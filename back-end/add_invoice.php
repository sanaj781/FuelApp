    ...
    if (isset($_POST['add_document'])) {
        if (isset($_FILES['image']) && !empty($_POST['oddometer']) && is_numeric($_POST['oddometer']) && !empty($_POST['amount']) && is_numeric($_POST['amount'])) {
            if (!empty($_POST['position'])) {
                ...
                if ($_SESSION['car_oddometer'] > validate($_POST['oddometer'])) {
                    header("Location:../index.php?page=add-document&status=error&error=Przebieg nie może byc mniejszy niż ostatnio zadeklarowany");
                } else {
                    $errors = array();
